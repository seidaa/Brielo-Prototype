        ...
        {joinedMoves.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
            <div className="w-20 h-20 rounded-2xl bg-white/5 border border-white/8 flex items-center justify-center mb-5">
              <MessageCircle className="w-9 h-9 text-gray-600" strokeWidth={1.5} />
            </div>
            <h2 className="text-xl font-black text-white mb-2">No active Move Chats</h2>
            <p className="text-sm text-gray-500 max-w-[200px] mb-6">
              Join a Move to unlock the chat.
            </p>
            ...
            <Link href="/discover" className="mt-6">
              <span className="text-sm font-bold text-primary">Find Moves →</span>
            </Link>
          </div>
        ) : (
                ...
                <Link key={move.id} href={`/chat/${move.id}`}>
                  <div className={cn(
                    ...
                    <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", cat.color)}>
                      {CatIcon
                        ? <CatIcon className={cat.text} strokeWidth={1.75} style={{ width: 22, height: 22 }} />
