  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] pb-28 pt-14">
      <header className="fixed top-0 left-0 right-0 max-w-sm mx-auto bg-[#0d0d0d]/95 backdrop-blur-xl z-40 px-4 h-14 flex items-center border-b border-white/5">
        ...
        <h1 className="text-lg font-black text-white flex-1">Your Crews</h1>
      ...
      <div className="p-4">
        <p className="text-xs text-gray-500 mb-4">People you make moves with regularly.</p>

        <div className="space-y-3">
          {crews.map(crew => (
            <Link key={crew.id} href={`/circles/${crew.id}`}>
                ...
                <div className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <h2 className="font-black text-white text-sm leading-snug truncate">{crew.name}</h2>
                      <span className={cn(
                        ...
                        {crew.isPublic ? <Globe className="w-2.5 h-2.5" /> : <Lock className="w-2.5 h-2.5" />}
                        {crew.isPublic ? "Open" : "Private"}
                      </span>
                    </div>
                    <div className="flex items-center gap-3 text-[11px] text-gray-500">
                      <span className="flex items-center gap-1">
                        <Users className="w-3 h-3" /> {crew.membersCount} members
                      </span>
                      <span className="flex items-center gap-1 text-primary">
                        <Calendar className="w-3 h-3" /> {crew.nextMoveTime}
                      </span>
                    </div>
                  </div>
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-white/10 p-5 flex flex-col items-center text-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-white mb-1">Start a new crew</p>
            <p className="text-xs text-gray-500">Group your people for recurring moves.</p>
          </div>
          <Button className="bg-white/8 hover:bg-white/12 border border-white/10 text-white font-bold rounded-xl px-5 h-9 text-sm">
            <Plus className="w-4 h-4 mr-1.5" /> Create Crew
          </Button>
        </div>
      </div>
